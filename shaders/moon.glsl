...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
	...
	float a = dist2 < 1200.0 ? 1.0 : 0.0;
	float g = 0.9;
	float c3 = cnoise((uv + vec2(4000.0))/200.0);
	float c4 = cnoise((uv + vec2(6000.0))/50.0);
	if (dist2 < 1200.0 && (c1*2.0 + c2)/3.0 > 0.3) {
		g = 0.7;
	}
